...
class importController extends bdrecursosController
{
    ...
    public function __construct($lang, $url) 
    {
        ...
        $this->_excel = $this->loadModel('registrar','excel');
    }
    ...
    public function excel($recurso = false) 
    {
        ...
        $this->_view->assign('estandares', $this->_bdrecursos->getEstandar());

        //Modal Registrar Recurso
        if ($this->botonPress("bt_registrar")) 
        {
            $this->registrarRecurso("excel");
        }
        ...
        if ($recurso == 'registrar' && !empty($_FILES['archivo'])) 
        {
            ...
            $destino = ROOT_ARCHIVO_FISICO . "tmp/varios/" . $archivo;
            $_SESSION['archivo'] = $destino;
            // echo $_FILES['archivo']['tmp_name'];
            copy($_FILES['archivo']['tmp_name'], $destino);

            new PHPExcel();
            // echo "Hola1".$archivo;
            $objLector = PHPExcel_IOFactory::createReaderForFile($destino);
            // print_r($objLector);
            // echo "Hola2";
            $nombrehojas = $objLector->listWorksheetNames($destino);
            unset($objLector);
            // echo "Hola3";
            $idestandar = $this->_import->getRecurso($_SESSION['recurso']);
            // print_r($idestandar);exit;
    ...
    public function registrarRecurso($tipoServicio = false){
            ...
            $this->getTexto("tb_origen_recurso") && $this->getTexto("sl_estandar_recurso")) 
        {
            ...
            {   
                ...
                if ($idrecurrso[0] > 0) 
                {
                    $this->redireccionar("bdrecursos/import/".$tipoServicio."/" . $idrecurrso[0]);
                } 
                else 
                {
    ...
    {
        $carpetaDestino = ROOT_ARCHIVO_FISICO ;
    ...
    public function txt($recurso = false) 
    {
        ...
        $objPHPExcel = $objLector->load($destino);
        $valores_final = array();
        $valores_celda = array();
        ...
        if ($this->botonPress("bt_registrar")) 
        {
                ...
                $this->getTexto("tb_origen_recurso") && $this->getTexto("sl_estandar_recurso")) 
            {
                ...
                {   
                    ...
                    if ($idrecurrso[0] > 0) 
                    {
                        $this->redireccionar("bdrecursos/import/excel/" . $idrecurrso[0]);
                    } 
                    else 
                    {
                        $this->_view->assign('_error', 'No se pudo registrar el recurso: ' . $idrecurrso);
    ...
    public function rss($recurso = false)
    {
        ...
        $this->_view->assign('tipoServicio', 'rss');
        $this->_view->assign('idioma', Cookie::lenguaje());
        $this->_view->assign('idiomas',$this->_estandar->getIdiomas());        
        $this->_view->assign('estandares', $this->_bdrecursos->getEstandar());
        ...
        {            
            ...
            else
            {
                ...
                // print_r($feed->get_items());exit;
                
                foreach ($feed->get_items() as $item) 
                {                  
                        if ($_POST['Dub_Titulo'] != '') {
                            $Dub_Titulo = $_POST['Dub_Titulo'];
                        } else {
                            // echo $_POST['Dub_Titulo_s'];
                            // echo $item->get_title();
                            $Dub_Titulo = $this->seleccionarItemRSS($item,"Dub_Titulo_s");
                        }
                        if ($_POST['Arf_PosicionFisica'] != '') {
                            $Arf_PosicionFisica = $_POST['Arf_PosicionFisica'];
                        }else {
                            $Arf_PosicionFisica = $this->seleccionarItemRSS($item,'Arf_PosicionFisica');
                        }
                        if ($_POST['Pai_Nombre'] != '') {
                            $Pai_Nombre = $_POST['Pai_Nombre'];
                        }else {
                            $Pai_Nombre = $this->seleccionarItemRSS($item,'Pai_Nombre_s');
                        }
                        if ($_POST['Arf_URL'] != '') {
                            $Arf_URL = $_POST['Arf_URL'];
                        }else {
                            $Arf_URL = $this->seleccionarItemRSS($item,'Arf_URL_s');
                        }
                        if ($_POST['Aut_Nombre'] != '') {
                            $Aut_Nombre = $_POST['Aut_Nombre'];
                        }else {
                            $Aut_Nombre = $this->seleccionarItemRSS($item,'Aut_Nombre_s');
                        }
                        if ($Dub_Titulo != '' and ($Arf_PosicionFisica != '' or $Arf_URL != '') and $Pai_Nombre != '' and $Aut_Nombre != '') {
                            
                            if ($_POST['Dub_Descripcion'] != '') {
                                $Dub_Descripcion = $_POST['Dub_Descripcion'];
                            }else {
                                $Dub_Descripcion = $this->seleccionarItemRSS($item,'Dub_Descripcion_s');
                            }
                            if ($_POST['Dub_Editor'] != '') {
                                $Dub_Editor = $_POST['Dub_Editor'];
                            }else {
                                $Dub_Editor = $this->seleccionarItemRSS($item,'Dub_Editor_s');
                            }

                            if ($_POST['Dub_Colaborador'] != '') {
                                $Dub_Colaborador = $_POST['Dub_Colaborador'];
                            }else {
                                $Dub_Colaborador = $this->seleccionarItemRSS($item,'Dub_Colaborador_s');
                            }
                            if ($_POST['Dub_FechaDocumento'] != '') {
                                $Dub_FechaDocumento = $_POST['Dub_FechaDocumento'];
                            }else {
                                $Dub_FechaDocumento = $this->seleccionarItemRSS($item,'Dub_FechaDocumento_s');
                            }


                            if ($_POST['Dub_Formato'] != '') {
                                $Dub_Formato = $_POST['Dub_Formato'];
                            }else {
                                $Dub_Formato = $this->seleccionarItemRSS($item,'Dub_Formato_s');
                            }
                            if ($_POST['Dub_Identificador'] != '') {
                                $Dub_Identificador = $_POST['Dub_Identificador'];
                            }else {
                                $Dub_Identificador = $this->seleccionarItemRSS($item,'Dub_Identificador_s');
                            }
                            if ($_POST['Dub_Fuente'] != '') {
                                $Dub_Fuente = $_POST['Dub_Fuente'];
                            }else {
                                $Dub_Fuente = $this->seleccionarItemRSS($item,'Dub_Fuente_s');
                            }
                            if ($_POST['Dub_Idioma'] != '') {
                                $Dub_Idioma = $_POST['Dub_Idioma'];
                            }else {
                                $Dub_Idioma = $this->seleccionarItemRSS($item,'Dub_Idioma_s');
                            }
                            if ($_POST['Dub_Relacion'] != '') {
                                $Dub_Relacion = $_POST['Dub_Relacion'];
                            }else {
                                $Dub_Relacion = $this->seleccionarItemRSS($item,'Dub_Relacion_s');
                            }

                            if ($_POST['Dub_Cobertura'] != '') {
                                $Dub_Cobertura = $_POST['Dub_Cobertura'];
                            }else {
                                $Dub_Cobertura = $this->seleccionarItemRSS($item,'Dub_Cobertura_s');
                            }
                            if ($_POST['Dub_Derechos'] != '') {
                                $Dub_Derechos = $_POST['Dub_Derechos'];
                            }else {
                                $Dub_Derechos = $this->seleccionarItemRSS($item,'Dub_Derechos_s');
                            }
                            if ($_POST['Dub_PalabraClave'] != '') {
                                $Dub_PalabraClave = $_POST['Dub_PalabraClave'];
                            }else {
                                $Dub_PalabraClave = $this->seleccionarItemRSS($item,'Dub_PalabraClave_s');
                            }
                            if ($_POST['Aut_Profesion'] != '') {
                                $Aut_Profesion = $_POST['Aut_Profesion'];
                            }else {
                                $Aut_Profesion = $this->seleccionarItemRSS($item,'Aut_Profesion_s');
                            }
                            if ($_POST['Aut_Email'] != '') {
                                $Aut_Email = $_POST['Aut_Email'];
                            }else {
                                $Aut_Email = $this->seleccionarItemRSS($item,'Aut_Email_s');
                            }
                            if ($_POST['Tid_Descripcion'] != '') {
                                $Tid_Descripcion = $_POST['Tid_Descripcion'];
                            }else {
                                $Tid_Descripcion = $this->seleccionarItemRSS($item,'Tid_Descripcion_s');
                            }
                            if ($_POST['Ted_IdTemaDublin'] != '') {
                                $Ted_Descripcion = $_POST['Ted_IdTemaDublin'];
                            }else {
                                $Ted_Descripcion = $this->seleccionarItemRSS($item,'Ted_IdTemaDublin_s');
                            }

                            // echo "Formato:".$Dub_Formato."Aut_Nombre:".$Aut_Nombre;
                            ...
                            if (empty($tipodublin)) {
                                if (empty($Tid_Descripcion)) {
                                    $Tid_Descripcion = "Desconocido";
                                }
                            ...
                            if (empty($temadublin)) {
                                if (empty($Ted_Descripcion)) {
                                    $Ted_Descripcion = "Desconocido";
                                }
                                // echo "sipuesesooooo".$Idi_IdIdioma;
                                $temadublin = $this->_import->registrarTemaDublin(ucwords(strtolower($Ted_Descripcion)), $Idi_IdIdioma);
                                // print_r($temadublin);
                            }
                            $nombrearchivoUrl = "";
                            $nombrearchivo = $this->_import->getArchivoFisico($Arf_PosicionFisica);
                            $nombrearchivoUrl = $this->_import->getArchivoFisicoURL($Arf_URL);
                            if (!empty($nombrearchivo) || !empty($nombrearchivoUrl)) {
                                if (empty($nombrearchivo)) {

                                    $this->_import->actualizarArchivoFisico($formato[0], $nombrearchivoUrl['Arf_IdArchivoFisico'], $Arf_URL);
                                } else {
                                    $this->_import->actualizarArchivoFisico($formato[0], $nombrearchivo['Arf_IdArchivoFisico'], $Arf_URL);
                                    //falta cuando los dos esten llenos
                                }
                            } else {
                                if (empty($Arf_URL)) {
                                    $Arf_URL = "Desconocido";
                                    if (empty($Arf_PosicionFisica)) {
                                        $Arf_PosicionFisica = "Desconocido"; 
                                        $url = $this->_import->registrarArchivoFisico($Arf_PosicionFisica, $formato[0], $Dub_FechaDocumento); 
                                        //falta aca cuando ambos esten vacios
                                    } else {
                                        $url = $this->_import->registrarArchivoFisico($Arf_PosicionFisica, $formato[0], $Dub_FechaDocumento);
                                    }
                                } else {
                                    if (empty($Arf_PosicionFisica)) {
                                        $Arf_PosicionFisica = "Desconocido";
                                        $url = $this->_import->registrarUrlArchivoFisico($Arf_URL, $formato[0], $Dub_FechaDocumento);

                                    } else {
                                        $url = $this->_import->registrarArchivoFisico($Arf_PosicionFisica, $formato[0], $Dub_FechaDocumento);
                                        //falta aca cuando ambos esten llenos
                                    }                                
                                }
                            }
                            // echo $Arf_URL."////".$Arf_PosicionFisica;
                            $url = $this->_import->getArchivoFisicoURL($Arf_URL);
                            // print_r($url);
                            if (!empty($url)) {
                                $dublin = $this->_import->getDublinCore($Dub_Titulo, $tipodublin[0]);
                                if (empty($dublin) && !empty($Dub_Titulo)) {
                                    $dublin = $this->_import->registrarDublinCore($Dub_Titulo, $Dub_Descripcion, $Dub_Editor, $Dub_Colaborador, $Dub_FechaDocumento, $formato[0], $Dub_Identificador, $Dub_Fuente, $Dub_Idioma, $Dub_Relacion, $Dub_Cobertura, $Dub_Derechos, $Dub_PalabraClave, $tipodublin[0], $_SESSION['recurso'], $Idi_IdIdioma, $url['Arf_IdArchivoFisico'], $temadublin[0]);
                                        ...
                                        foreach ($Pai_Nombre as $Pai_Nombre) {
                                            ...
                                            if (!empty($pais)) {
                                                if (isset($dublin) and isset($pais)) {
                                                    $documentorelacionado = $this->_import->registrarDocumentosRelacionados($dublin[0], $pais['Pai_IdPais']);
                                                }
                                            }                                        
                                        }
                                    }
            ...
            //             $insertar = "INSERT INTO " . $tabla . " VALUES ( null, script "
            ...
            $descarga = $_SESSION['Descargar'];
            $cabecera = array("Dub_Titulo","Dub_Descripcion","Dub_Editor","Dub_Colaborador","Dub_FechaDocumento","Dub_Formato","Dub_Identificador","Dub_Fuente","Idi_IdIdioma","Dub_Relacion","Dub_Cobertura","Dub_Derechos","Dub_PalabraClave","Pai_Nombres","Tid_Descripcion","Aut_Nombre","Aut_Profesion","Aut_Email","Arf_PosicionFisica","Ted_Descripcion","Arf_URL");
            $_SESSION['cabecera'] = $cabecera;
            $_SESSION['datos_no_registrados'] = $descarga;
            $this->_view->assign('total_registrado', $_SESSION['total_registrado']);
            $this->_view->assign('total_no_registrado', $_SESSION['total_no_registrado']);
            $this->_view->assign('no_registrado', $paginador->paginar($_SESSION['datos_no_registrados'], "", "", "", 250));
            $this->_view->assign('paginacion', $paginador->getView('paginacion_ajax'));
            $this->_view->assign('total_registros', $_SESSION['total_registros']);
            $this->_view->assign('cabecera', $_SESSION['cabecera']);
            unset($descarga);
            ...
            Session::destroy('cabecera');
            Session::destroy('datos_no_registrados');
            Session::destroy('total_registros');
            Session::destroy('total_registrado');
            Session::destroy('total_no_registrado');
    ...
    public function seleccionarItemRSS($item,$Fie_ColumnaTabla){
        // echo "jvhadjfgvadugvsafuvlkjdbvlhdfjhadjvkfhakdjfvhlkdjfvhkjdafhvlkadfhlkj";
        // $fichaEstandar = $this->_import->getFichaEstandar($_SESSION['idestandar'], Cookie::lenguaje());
        // // print_r($fichaEstandar);
        // foreach ($fichaEstandar as $ficha) 
        // {
        // echo $_POST[$Fie_ColumnaTabla."_s"];
            if($_POST[$Fie_ColumnaTabla] != 's_')
            {                
                if ($_POST[$Fie_ColumnaTabla] == 's_titulo') 
                {
                    return $item->get_title();
                }
                else if($_POST[$Fie_ColumnaTabla] == 's_categoria')
                {
                  return$item->get_category();
                }
                else if($_POST[$Fie_ColumnaTabla] == 's_descripcion')
                {
                  return $item->get_description();                  
                } 
                else if($_POST[$Fie_ColumnaTabla] == 's_contenido')
                {
                  return $item->get_content();
                }                    
                else if($_POST[$Fie_ColumnaTabla] == 's_autor')
                {
                  return $item->get_author();
                }
                else if($_POST[$Fie_ColumnaTabla] == 's_contribuyente')
                {
                  return $item->get_contributor();
                }
                else if($_POST[$Fie_ColumnaTabla] == 's_derechos_autor')
                {
                  return $item->get_copyright();
                }
                else if($_POST[$Fie_ColumnaTabla] == 's_fecha')
                {
                  return $item->get_date();
                }
                else if($_POST[$Fie_ColumnaTabla] == 's_link')
                {
    ...
    public function json($recurso = false)
    {
        ...
        $this->_view->assign('tipoServicio', 'json');
        $this->_view->assign('idioma', Cookie::lenguaje());
        $this->_view->assign('idiomas',$this->_estandar->getIdiomas());
        $this->_view->assign('estandares', $this->_bdrecursos->getEstandar());
        ...
        {            
            $nombre_tabla = $fichaEstandar[0]['Fie_NombreTabla'];
            ...
            else
            {
                ...
                //     foreach ($fichaEstandar as $ficha) 
                //     {
                //         print_r($ficha);print_r($item); echo "<br>";
                //         if($ficha['Fie_TipoDatoCampo'] == "int" || $ficha['Fie_TipoDatoCampo'] == "double") 
                //         {
                ...
                //             }
                //         }
                //         if ($fich2a['Fie_TipoDatoCampo'] == "varchar") 
                //         {
                ...
                foreach ($array as $objCelda) {
                    if ($_POST['Dub_Titulo'] != '') {
                        $Dub_Titulo = $objCelda[$_POST['Dub_Titulo']];
                    }
                    // if ($_POST['Arf_PosicionFisica'] != '') {
                    //     $Arf_PosicionFisica = $objCelda[$_POST['Arf_PosicionFisica']];
                    // }
                    if ($_POST['Pai_Nombre'] != '') {
                    ...
                    // echo $_POST['Dub_Titulo']." : Dub_Titulo : ".$Dub_Titulo;                    
                    // echo $_POST['Arf_PosicionFisica']." : Arf_PosicionFisica : ".$Arf_PosicionFisica;
                    // echo $_POST['Arf_URL']." : Arf_URL : ".$Arf_URL;
                    // echo $_POST['Aut_Nombre']." : Aut_Nombre : ".$Aut_Nombre;
                    // echo $_POST['Dub_Descripcion']." : Dub_Descripcion :".$Dub_Descripcion;
                    // echo $_POST['Dub_Editor']." : Dub_Editor : ".$Dub_Editor;
                    // echo $_POST['Dub_Titulo']." : ".$Dub_Titulo;
                    // echo $_POST['Dub_Titulo']." : ".$Dub_Titulo;
                    // echo $_POST['Dub_Titulo']." : ".$Dub_Titulo;
                    // echo $_POST['Dub_Titulo']." : ".$Dub_Titulo;
                    // echo $_POST['Dub_Titulo']." : ".$Dub_Titulo;
                    ...
                    // echo $_POST['Dub_Titulo']." : ".$Dub_Titulo;
                    // echo $_POST['Dub_Titulo']." : ".$Dub_Titulo;
                    // echo($Dub_Titulo." : Dub_Titulo : ".$Arf_PosicionFisica."AAA".$Arf_URL."BBB".$Pai_Nombre."CCCC".$Aut_Nombre);
                    if ($Dub_Titulo != '' and ($Arf_PosicionFisica != '' or $Arf_URL != '') and $Pai_Nombre != '' and $Aut_Nombre != '') {
                        // echo "acacaca";
                        if ($_POST['Dub_Descripcion'] != '') {
                        ...
                        if (!empty($url)) {
                            ...
                            if (empty($dublin) && !empty($Dub_Titulo)) {
                                ...
                                $total_no_registrado++;
                                // $datos[$total_no_registrado] = array_keys($objCelda[$_POST['Dub_Titulo']]);
                                $datos[$total_no_registrado][0] = $objCelda[$_POST['Dub_Titulo']];
                                $datos[$total_no_registrado][1] = $objCelda[$_POST['Dub_Descripcion']];
                                $datos[$total_no_registrado][2] = $objCelda[$_POST['Dub_Editor']];
                                $datos[$total_no_registrado][3] = $objCelda[$_POST['Dub_Colaborador']];
                                $datos[$total_no_registrado][4] = $objCelda[$_POST['Dub_FechaDocumento']];
                                $datos[$total_no_registrado][5] = $objCelda[$_POST['Dub_Formato']];
                                $datos[$total_no_registrado][6] = $objCelda[$_POST['Dub_Identificador']];
                                $datos[$total_no_registrado][7] = $objCelda[$_POST['Dub_Fuente']];
                                $datos[$total_no_registrado][8] = $objCelda[$_POST['Dub_Idioma']];
                                $datos[$total_no_registrado][9] = $objCelda[$_POST['Dub_Relacion']];
                                $datos[$total_no_registrado][10] = $objCelda[$_POST['Dub_Cobertura']];
                                $datos[$total_no_registrado][11] = $objCelda[$_POST['Dub_Derechos']];
                                $datos[$total_no_registrado][12] = $objCelda[$_POST['Dub_PalabraClave']];
                                $datos[$total_no_registrado][13] = $objCelda[$_POST['Pai_Nombre']];
                                $datos[$total_no_registrado][14] = $objCelda[$_POST['Tid_Descripcion']];
                                $datos[$total_no_registrado][15] = $objCelda[$_POST['Aut_Nombre']];
                                $datos[$total_no_registrado][16] = $objCelda[$_POST['Aut_Profesion']];
                                $datos[$total_no_registrado][17] = $objCelda[$_POST['Aut_Email']];
                                // $datos[$total_no_registrado][18] = $objCelda[$_POST['Arf_PosicionFisica']];
                                $datos[$total_no_registrado][18] = $objCelda[$_POST['Ted_IdTemaDublin']];
                                $datos[$total_no_registrado][19] = $objCelda[$_POST['Arf_URL']];

                            }
                        } else {
                            $total_no_registrado++;
                            // $datos[$total_no_registrado] = array_keys($objCelda);
            ...
            // print_r($datos);
            $descarga = $_SESSION['Descargar'];
            $cabecera = array('Dub_Titulo',"Dub_Descripcion","Dub_Editor","Dub_Colaborador","Dub_FechaDocumento","Dub_Formato","Dub_Identificador","Dub_Fuente","Dub_Idioma","Dub_Relacion","Dub_Cobertura","Dub_Derechos","Dub_PalabraClave","Pai_Nombre","Tid_Descripcion","Aut_Nombre","Aut_Profesion","Aut_Email","Ted_Descripcion","Arf_URL");
            $_SESSION['cabecera'] = $cabecera;
            $_SESSION['datos_no_registrados'] = $descarga;
            // print_r($_SESSION['datos_no_registrados']);
            $this->_view->assign('total_registrado', $_SESSION['total_registrado']);
            $this->_view->assign('total_no_registrado', $_SESSION['total_no_registrado']);
            $this->_view->assign('no_registrado', $paginador->paginar($_SESSION['datos_no_registrados'], "", "", "", 250));
            $this->_view->assign('paginacion', $paginador->getView('paginacion_ajax'));
        ...
        if (!empty($_SESSION['tipo_archivo'])) {
            ...
            foreach ($objHoja as $iIndice => $objCelda) {
                ...
                if ($Dub_Titulo != '' and ( $Arf_PosicionFisica != '' or $Arf_URL != '') and $Pai_Nombre != '' and $Aut_Nombre != '') {
                    ...
                    if (!empty($nombrearchivo)) {
                        $this->_import->actualizarArchivoFisico($formato['Taf_IdTipoArchivoFisico'], $nombrearchivo['Arf_IdArchivoFisico'], $Arf_URL);
                    } else {
                        $url = $this->_import->registrarUrlArchivoFisico($Arf_URL, $formato['Taf_IdTipoArchivoFisico']);
                    }

                    $url = $this->_import->getArchivoFisicoURL($Arf_URL);
                    // $url;exit;
                    if (!empty($url)) {
                        $dublin = $this->_import->getDublinCore($Dub_Titulo, $tipodublin['Tid_IdTipoDublin']);

                        if (empty($dublin)) {
                            $dublin = $this->_import->registrarDublinCore($Dub_Titulo, $Dub_Descripcion, $Dub_Editor, $Dub_Colaborador, $Dub_FechaDocumento, $formato['Taf_Descripcion'], $Dub_Identificador, $Dub_Fuente, $Dub_Idioma, $Dub_Relacion, $Dub_Cobertura, $Dub_Derechos, $Dub_PalabraClave, $tipodublin['Tid_IdTipoDublin'], $_SESSION['recurso'], $Idi_IdIdioma, $url['Arf_IdArchivoFisico'], $temadublin['Ted_IdTemaDublin']);
    ...
    public function _invocarWS() 
    {
        ...
        // echo $function."//".$urlws."//".$idrecurso."//".$r_tabla;
        // print_r($parametros);
        ...
        try 
        {
            ...
            if (count($array)) 
            {
                $columnas = array();
                $entidad = array();
                $arr = get_object_vars($arr["Dublin"][0]);
            ...
            //     $xml = simplexml_load_string($ready);
            //     $columnas = array();
            //     $entidad = array();
            ...
            //         $array = json_decode($json, TRUE);
            //     }
                // $columnas = array("dub","d1","d2","d3");
                // $entidad = array("enti","en1","en2","en3");
