<div id="recursos" class="container-fluid" >
    <div class="row">
        ...
        <!-- Registrar Datos Archivos -->
        <div class="col-md-12">  
            {if $_acl->permiso("agregar_recurso") || (isset($recurso) && $_acl->permiso("editar_recurso"))}
                <div class="panel panel-default">
                    <div class="panel-heading ">                    
                        <h3 aria-expanded="false" data-toggle="collapse" href="#collapseImportar" class="panel-title collapsed">
                            <i style="float:right" class="fa fa-ellipsis-v"></i><i class=" glyphicon glyphicon-book"></i>&nbsp;&nbsp;<strong>Registrar/Importar Datos</strong></h3>       
                    </div>
                    <div aria-expanded="false" id="collapseImportar" class="panel-collapse collapse {if isset($recurso)}in{/if}">
                        <div class="panel-body">

                            <div class="pull-left">
                                <div class="form-group">
                                    <label  class="col-lg-2 control-label" for="tb_origen_recurso">Recurso*</label>
                                    <div class="col-lg-10">
                                         <select name="sl_recurso"  class="form-control" id="sl_recurso" required="">
                                            <option value="">Seleccione Recurso</option>
                                            {if isset($recursosCompletos) && count($recursosCompletos)}
                                                {foreach item=dat from=$recursosCompletos}
                                                    <option value='{$dat.Rec_IdRecurso}'> {$dat.Rec_Nombre}</option>
                                                {/foreach}
                                            {/if}
                                        </select>

                                        <!-- <input type="text" list="dl_resursos" class="form-control" id="tb_recurso_" value="{$recurso.Rec_Origen|default}" name="tb_recurso_"
                                               placeholder="Seleccione Recurso" required/>
                                        <datalist id="dl_resursos">
                                            {if isset($recursosCompletos) && count($recursosCompletos)}
                                                {foreach item=dat from=$recursosCompletos}
                                                    <option value='{$dat.Rec_Nombre}'> {$dat.Rec_Nombre}</option>
                                                {/foreach}
                                            {/if}
                                        </datalist> -->
                                    </div>
                                </div>
                                <!-- <div id = "registrar_importar_datos">
                                ...
                                <tbody>
                                    {foreach item=datos from=$registros}
                                        <tr>                       
                                            ...
                                            <td style=" text-align: center">
                                                <a type="button" title="{$lenguaje["ver_bdrecursos"]}" class="btn btn-default btn-sm glyphicon glyphicon-search" href="{$_layoutParams.root}bdrecursos/metadata/index/{$datos.Rec_IdRecurso}" target="_blank">
                                                </a>
                                                {if $_acl->permiso("editar_recurso")}
                                                    <a type="button" title="{$lenguaje["editar_bdrecursos"]}" class="btn btn-default btn-sm glyphicon glyphicon-pencil" href="{$_layoutParams.root}bdrecursos/index/index/{$datos.Rec_IdRecurso}" {if $datos.Tir_IdTipoRecurso ==2}target="_blank"{/if}>
                                                    </a>
                                                {/if}
