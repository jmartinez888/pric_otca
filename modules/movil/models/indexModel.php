...
class indexModel extends Model {
    ...
    public function getContenido($Lec_IdLeccion){
        $sql = $this->_db->query(
                "SELECT CL_IdContenido,CL_Descripcion FROM contenido_leccion WHERE Lec_IdLeccion=$Lec_IdLeccion"
        );              
        return $sql->fetchAll();
    }   
    ...
    public function getCursosUsuario($Usu_IdUsuario,$Con_Descripcion,$Cur_Titulo){
        $sql = $this->_db->query(
                ...
                , COUNT(vc.Usu_IdUsuario) AS Valoraciones, 
                    CAST((CASE WHEN AVG(vc.Val_Valor) > 0 THEN AVG(vc.Val_Valor) ELSE 0 END) AS DECIMAL(2,1)) AS Valor
                FROM
    ...
    public function getDocenteCurso($Cur_IdCurso){          
        $sql = $this->_db->query(
            "SELECT Usu_IdUsuario,CONCAT(Usu_Nombre,' ',Usu_Apellidos) AS Usu_Nombres,Usu_URLImage,Usu_GradoAcademico,Usu_Especialidad,Usu_Perfil,Usu_InstitucionLaboral,Usu_Cargo FROM usuario WHERE Usu_Estado = 1 AND Row_Estado = 1
