...
class gestionController extends elearningController {
  ...
  public function anuncios($id ="",$tipo=0){
    ...
    //Filtro por Activos/Eliminados
        ...
        if($tipo>0)
            $condicion = " WHERE Cur_IdCurso = $id and Anc_Estado=1 and Row_Estado=1 order by Anc_FechaReg desc ";
        // echo $condicion; exit;
   ...
   public function _paginacion_listaranuncios($txtBuscar = false) 
    {
        ...
        if($tipo>0)
            if ($txtBuscar) 
                $condicion = " WHERE Cur_IdCurso = $idCurso AND Anc_Titulo liKe '%$txtBuscar%' and Anc_Estado=1 and Row_Estado=1 order by Anc_FechaReg desc ";
            else 
                $condicion = " WHERE Cur_IdCurso = $idCurso and Anc_Estado=1 and Row_Estado=1 order by Anc_FechaReg desc ";
 ...
 public function _cambiarEstadoAnuncio()
    {
        ...
        if($tipo>0)
            if ($txtBuscar) 
                $condicion = " WHERE Cur_IdCurso = $id AND Anc_Titulo liKe '%$txtBuscar%' and Anc_Estado=1 and Row_Estado=1 order by Anc_FechaReg desc ";
            else 
                $condicion = " WHERE Cur_IdCurso = $id and Anc_Estado=1 and Row_Estado=1 order by Anc_FechaReg desc ";
