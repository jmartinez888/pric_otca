...
class cursosController extends elearningController {
  ...
  public function miscursos_docucente(){
    if(!Session::get("autenticado")){ $this->redireccionar("elearning/"); }
    $model = $this->loadModel("curso");
    $mConstante = $this->loadModel("constante");
  ...
  public function curso_dirigido($id = ""){
    ...
    $matriculado = (Session::get("autenticado") && isset($inscripcion) && count($inscripcion)>0 && $inscripcion[0]["Mat_Valor"]==1);
    ...
    $this->_view->assign("session", Session::get("autenticado"));
    $this->_view->assign("matriculado", $matriculado);
  ...
  public function modulo($curso= "", $modulo = "", $leccion = ""){
    ...
    if($ocurso["Usu_IdUsuario"]!=Session::get("id_usuario")){
    ...
    if($OLeccion["Lec_Tipo"] == 1){
        ...
        if($tmp["Progreso"]==1){
          $this->redireccionar("elearning/cursos/modulo/" . $curso. "/" . $tmp["Mod_IdModulo"] . "/" . $lecciones[0]["Lec_IdLeccion"]);
        }
  ...
  public function _previous_leccion(){
    ...
    if($ocurso[0]["Usu_IdUsuario"]==Session::get("id_usuario")){
  ...
  public function _next_leccion(){
    ...
    $ocurso = $Cmodel->getCursoID($curso);
    if($ocurso==null || count($ocurso)==0){ $this->redireccionar("elearning/"); }
  ...
  public function _send_examen(){
    ...
    $OBJ_PREGS = $model->getExamen($OBJ_EXAMEN["Lec_IdLeccion"]);
    $intentos = $model->getIntentos($OBJ_EXAMEN["Lec_IdLeccion"], Session::get("id_usuario"));
    ...
    foreach ($preguntas as $p) {
      ...
      if ($preg["Pre_Valor"] != $p->Respuesta){
        ...
        array_push($ERRORES, $preg);
      }
      $model->insertRespuesta(Session::get("id_usuario"), $p->IdPregunta, $p->Respuesta);
    }
    $porcentaje = $OBJ_EXAMEN["Exa_Porcentaje"];
    $preguntas_totales = $model->getPreguntas($OBJ_EXAMEN["Exa_IdExamen"]);
    $resueltas = (COUNT($preguntas_totales)-count($ERRORES));
    $proporcion = $resueltas==0 ? 0 : ( ($resueltas * 100) / COUNT($preguntas_totales));

    if( $proporcion >= $porcentaje){
      $Lmodel->RegistrarProgreso($OBJ_EXAMEN["Lec_IdLeccion"], Session::get("id_usuario"));
    }
    $resultado = array( "ERRORES" => $ERRORES, "Porcentaje" => $proporcion);

    $json->Populate($resultado);
    if( count($ERRORES) > 0 &&  ($proporcion < $porcentaje) ){
      $json->Error("Hay preguntas incorrectas");
    }else{
