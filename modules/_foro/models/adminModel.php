...
class adminModel extends Model {
    ...
    public function insertarForo($iFor_Titulo, $iFor_Objetivo, $iFor_ResultadosEsperados, $iFor_Palabras, $iFor_FechaCierre,$iFor_Tipo, $iFor_Estado, $iLit_IdLineaTematica, $iUsu_IdUsuario,$iEnt_IdEntidad,$iRec_IdRecurso,$iIdi_IdIdioma) {
        try {
            $sql = " call s_i_foro(?,?,?,?,?,?,?,?,?,?,?,?)";
            $result = $this->_db->prepare($sql);
            $result->bindParam(1, $iFor_Titulo, PDO::PARAM_STR);
            $result->bindParam(2, $iFor_Objetivo, PDO::PARAM_STR); 
            $result->bindParam(3, $iFor_ResultadosEsperados, PDO::PARAM_STR);
            $result->bindParam(4, $iFor_Palabras, PDO::PARAM_STR);
            $result->bindParam(5, $iFor_FechaCierre, PDO::PARAM_STR);
            $result->bindParam(6, $iFor_Tipo, PDO::PARAM_INT);
            $result->bindParam(7, $iFor_Estado, PDO::PARAM_INT);
            $result->bindParam(8, $iLit_IdLineaTematica, PDO::PARAM_INT);
            $result->bindParam(9, $iUsu_IdUsuario, PDO::PARAM_INT);
            $result->bindParam(10, $iEnt_IdEntidad, PDO::PARAM_INT);
             $result->bindParam(11, $iRec_IdRecurso, PDO::PARAM_INT);
            $result->bindParam(12, $iIdi_IdIdioma, PDO::PARAM_STR);
    ...
    public function getRowForos($iFor_Filtros = "") {
        try {
            $post = $this->_db->query(
                    "SELECT COUNT(*) as For_NRow from foro f WHERE f.For_Titulo LIKE '%$iFor_Filtros%' OR f.For_Objetivo LIKE '%$iFor_Filtros%' OR f.For_ResultadosEsperados LIKE '%$iFor_Filtros%' OR f.For_PalabrasClaves LIKE '%$iFor_Filtros%'");
