	                ...
	                <div class="col-sm-8">
	                    <form class="form-horizontal" data-toggle="validator"  role="form" @submit.prevent="onSubmit_registrar" novalidate="true">
	                        ...
	                        {foreach $idiomas as $idioma}
		                        <div class="form-group">
		                            <label class="col-lg-2 control-label" for="nombre">{$idioma->Idi_IdIdioma} : </label>
		                            <div class="col-lg-10">
		                                <input type="text" name="" id="idioma_{$idioma->Idi_IdIdioma}" class="form-control" value="" required="required" v-model="idiomas.idioma_{$idioma->Idi_IdIdioma}">
		                            </div>
		                        </div>
	                        {/foreach}
    ...
    {else}
	    <div class="panel panel-default">
	        <div class="panel-body">
						<h3>{$lenguaje['str_elemento_no_encontrado']}</h3>
					</div>
			</div>
   	{/if}
...
{/block}
{block 'template'}
<template id="listar_body">
	<ul>
		{literal}{{#idiomas}}{/literal}
		<li>{literal}{{idioma_id}} : {{body}}{/literal}</li>
		{literal}{{/idiomas}}{/literal}
...
{if $elemento != null}
	...
	<script type="text/javascript" src="{$_layoutParams.rutas.js}detalles.js"></script>
{/if}
{/block}
